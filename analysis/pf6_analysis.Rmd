...
pairwise.wilcox.test(
  patient_lat_long$dis_var_med, 
  patient_lat_long$Subregion,
  p.adjust.method = "BH"
)

# Nemenyi test
pvalue <- PMCMRplus::kwAllPairsNemenyiTest(
  dis_var_med ~ as.factor(Continent), 
  data = patient_lat_long
)$p.value

purrr::map_df(tibble::as_tibble(pvalue), ~ round(.x, digits = 5)) %>% 
  tibble::add_column(Subregion = rownames(pvalue), .before = 1)
```


```{r violin plot}
ggpubr::compare_means(dis_var_med ~ Continent, data = patient_lat_long)
