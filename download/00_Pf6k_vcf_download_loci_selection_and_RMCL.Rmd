...
new_dir <- "/gpfs_home/owatson1/data/owatson1/vcfs/pf3k_release6_COI_filtering/"
...
setwd(file.path(here::here(), "analysis/data/slurm_outs/"))
...
sjob <- rslurm::slurm_apply(
  ...
  add_objects = c("bcf_sample_subset", "bcft", "obj_list"),
...
We'll continue by subsetting to the SNPs that have a minor allele frequency greater than 0.01.
...
for(i in seq_along(tfvars)) {
  if(!file.exists(tfvars[i])) {
    system(paste("module load bcftools/1.9; bcftools query -f '%POS\n' ", l[i], ">", tfvars[i]))
  }
}
...
sjob <- rslurm::slurm_apply(
  ...
  params = data.frame(i = seq_along(obj_list)),
  add_objects = c("sample_maf_subset_count", "obj_list"),
...
saveRDS(intersect_snps, file.path(here::here(),"analysis/data/derived/McCOIL_work/intercept_snps_0.01.rds"))
```
...
intersect_snps <- readRDS(
  file.path(here::here(),"analysis/data/derived/McCOIL_work/intercept_snps_0.01.rds")
  )
...
sjob <- rslurm::slurm_apply(
  ...
  params = data.frame(i = seq_along(obj_list)),
  add_objects = c("loci_subset", "obj_list"),
...
We will need to choose loci that are not in LD. This will also help bring down the number of loci to be analysed, which will help THE REAL McCOIL out a lot. 

We can start though by merging our individual chromosome VCFs into one, from which we will confuct LD filtering
...
vcf_rds <- file.path(here::here(),"analysis/data/derived/McCOIL_work/vcf_final.rds")
...
devtools::install_github("OJWatson/vcfRmanip")
library(vcfR)
...
gen_rds <- file.path(here::here(),"analysis/data/derived/McCOIL_work/gen.rds")
saveRDS(gen, gen_rds)
...
ld_filter <- function(vcfRobj_rds, genauto_rds, threshR2 = 0.8, random=TRUE){
  ...
  vcfRmanip::vcfR2LDfiltered(vcfR = vcfRobj, genautocorrresult = gen,
                             threshR2 = threshR2, random = random)
  
}

# parameter df
pars <- data.frame("vcfRobj_rds" = vcf_rds, 
                 "genauto_rds" = gen_rds, 
                 "threshR2" = rep(0.8, 10),
                 "random" = TRUE, 
...
Now that we have suitable datasets we need to split them up into their regions and then create our heterozygous calls for putting into RMCL

```{r}

# function to load vcf and siubset to samples in given region, create gt and then RMCL
submit_rmcl <- function(rep, vcf_rds, region, meta_df, path) {
  ...
  vcfRobj <- vcfRmanip::select_samples(vcfRobj, meta_df$Sample[meta_df$cluster == region])
  ...
  out <- McCOILR::McCOIL_categorical(data = gtmat,
                                     maxCOI = 25,
                                     totalrun = 100000,
                                     burnin = 1000,
                                     M0 = 5,
...
sjob <- rslurm::slurm_apply(
  ...
  params = data.frame(i = seq_along(obj_list)),
  add_objects = c("submit_rmcl", "obj_list"),
  jobname = 'RMCL',
  nodes = length(obj_list),
  cpus_per_node = 1,
  slurm_options = sopt,
